...
--==================== UI ====================--
local tab = Menu.Create("Scripts", "User Scripts", "Armlet Abuse")
tab:Icon("\u{f135}")

local gMain   = tab:Create("Principal"):Create("Geral")
local gThr    = tab:Create("Limiares"):Create("Geral")
local gSafe   = tab:Create("Segurança"):Create("Geral")
local gPred   = tab:Create("Predição"):Create("Geral")
local gBurst  = tab:Create("Anti-Explosão"):Create("Geral")
local gImm    = tab:Create("Imortalidade"):Create("Geral")
local gSumm   = tab:Create("Anti-Invocações"):Create("Geral")
local gDbg    = tab:Create("Debug"):Create("Logs")

local ui = {}
ui.enable      = gMain:Switch("Ativar Armlet Abuse", true)
ui.profile     = gMain:Combo("Perfil", {"Seguro", "Agressivo"}, 0)
ui.manual_allow= gMain:Switch("Permitir controle manual do Armlet (não forçar)", false)

ui.hp_thr      = gThr:Slider("Ligar automaticamente abaixo de HP%", 1, 40, 15, "%d")
ui.safe_hp     = gThr:Slider("HP de segurança (base)", 100, 1500, 480, "%d")
ui.enemy_rad   = gThr:Slider("Raio para checar inimigos", 600, 2400, 1300, "%d")

ui.dot_block   = gSafe:Switch("Impedir desligar sob DoT", true)
ui.tp_block    = gSafe:Switch("Forçar ligado enquanto teleportando/canalizando", true)
ui.rosh_threat = gSafe:Switch("Contar Roshan como ameaça", true)
ui.creep_threat= gSafe:Switch("Contar creeps como ameaça", false)

ui.proj_predict= gPred:Switch("Previsão de projéteis (alvo/AoE)", true)
ui.part_predict= gPred:Switch("Detectar riscos de partículas", true)
ui.proj_grace  = gPred:Slider("Tolerância após último projétil (ms)", 0, 3000, 950, "%d")

ui.burst_enable= gBurst:Switch("Ativar modo anti-burst", true)
ui.omni_radius = gBurst:Slider("Detectar Omnislash do Jugger", 400, 1800, 1000, "%d")

-- Núcleo de imortalidade
ui.imm_enable  = gImm:Switch("Modo imortalidade (histerese/memória)", true)
ui.hyst_ms     = gImm:Slider("Histerese para manter ligado (ms)", 200, 3500, 1200, "%d")
ui.mem_ms      = gImm:Slider("Janela de memória de ameaça (ms)", 300, 6000, 2000, "%d")
ui.safe_per_en = gImm:Slider("Margem de segurança por inimigo", 0, 300, 90, "%d")
ui.safe_proj   = gImm:Slider("Margem de segurança: projéteis ativos", 0, 600, 240, "%d")
ui.huskar_bias = gImm:Switch("Viés para Huskar (segurar ligado extra)", true)

-- Anti-invocações
ui.summ_enable   = gSumm:Switch("Ativar retenção anti-invocação", true)
ui.summ_radius   = gSumm:Slider("Raio para invocações", 300, 1600, 850, "%d")
ui.summ_min      = gSumm:Slider("Ativar com >= N invocações", 1, 15, 3, "%d")
ui.summ_safe_add = gSumm:Slider("Margem de segurança por invocação", 0, 180, 45, "%d")

-- Depuração
ui.debug_log     = gDbg:Switch("Registrar toggles no console", false)
ui.debug_particles= gDbg:Switch("Registrar impactos de partículas", false)
