...
local ABILITY_DEFINITIONS = {
    reactive_armor = {
        ability_names = { "shredder_reactive_armor", "timbersaw_reactive_armor" },
        icon = "panorama/images/spellicons/shredder_reactive_armor_png.vtex_c",
    ...
    phoenix_supernova = {
        ability_names = { "phoenix_supernova" },
        icon = "panorama/images/spellicons/phoenix_supernova_png.vtex_c",
        display_name = "Phoenix Supernova",
        type = "no_target",
        requires_enemy = true,
        search_range = 1200,
    },
    sniper_grenade = {
        ability_names = { "sniper_concussive_grenade" },
        icon = "panorama/images/spellicons/sniper_concussive_grenade_png.vtex_c",
        display_name = "Concussive Grenade",
        type = "position_enemy",
        range = 1800,
        requires_enemy = true,
        search_range = 1800,
        grenade_cast_offset = 400,
    },
}
...
for _, key in ipairs(priority_keys) do
    local definition = ITEM_DEFINITIONS[key] or ABILITY_DEFINITIONS[key]
    if definition then
...
for _, item in ipairs(priority_items) do
    ...
    if definition then
        ...
        if needs_enemy_range then
            ...
            -- Enemy count filter (min)
            local min_label = string.format("%s - Inimigos Mínimos", definition.display_name)
            ...
            apply_compact_style(item_enemy_min_counts[key])
            apply_widget_icon(item_enemy_min_counts[key], resolve_item_icon(definition))
        end
    end
end
...
local function can_use_item(hero, is_ability)
    if not Entity.IsAlive(hero) then
        return false
    end

    -- Always block if stunned or hexed
    if NPC.HasState(hero, Enum.ModifierState.MODIFIER_STATE_STUNNED) or NPC.HasState(hero, Enum.ModifierState.MODIFIER_STATE_HEXED) then
        return false
    end

    -- Items are blocked by MUTED; abilities by SILENCED
    if is_ability then
        if NPC.HasState(hero, Enum.ModifierState.MODIFIER_STATE_SILENCED) then
            return false
        end
    else
        if NPC.HasState(hero, Enum.ModifierState.MODIFIER_STATE_MUTED) then
            return false
        end
    end
...
local function get_hero_ability(hero, definition)
    if not definition then
        return nil
    end

    if definition.ability_name and type(definition.ability_name) == "string" then
        return NPC.GetAbility(hero, definition.ability_name)
    end

    if type(definition.ability_names) == "table" then
        for _, name in ipairs(definition.ability_names) do
            if type(name) == "string" then
                local ability = NPC.GetAbility(hero, name)
                if ability then
                    return ability
                end
            end
...
local function cast_phoenix_supernova(hero, ability)
    if not hero or not ability then
        return false
    end

    local has_scepter = NPC.HasScepter(hero)
    ...
    if has_scepter then
        -- Com Aghanim: envia ordem de cast em si mesmo (double-tap R)
        ...
        -- Sem Aghanim: envia ordem de cast sem alvo (apertar R simples)
        Player.PrepareUnitOrders(
            player,
...
local function cast_item(hero, item_key, game_time)
    local definition = ITEM_DEFINITIONS[item_key] or ABILITY_DEFINITIONS[item_key]
    if not definition then
    ...
    if is_ability then
        item = get_hero_ability(hero, definition)
    else
        item = get_inventory_item(hero, definition)
    end
    ...
    if definition.modifier and ITEM_DEFINITIONS[item_key] and NPC.HasModifier(hero, definition.modifier) then
        return CAST_RESULT_NONE
    end

    -- Abilities must be learned (level > 0)
    if is_ability then
        local level = Ability.GetLevel(item)
    ...
    local mana = NPC.GetMana(hero)
    local is_phoenix_supernova = (item_key == "phoenix_supernova")
    if is_phoenix_supernova and NPC.HasScepter(hero) then
        local cost = (Ability.GetManaCost and Ability.GetManaCost(item)) or 0
        if mana < cost then
            return CAST_RESULT_NONE
        end
        -- with scepter, ability becomes unit-target; we'll self-cast below
    else
        if not Ability.IsCastable(item, mana) then
            return CAST_RESULT_NONE
        end
    ...
    if not can_use_item(hero, is_ability) then
        return CAST_RESULT_NONE
    end
    ...
    if definition.requires_enemy then
        ...
        -- Apply minimum enemy count filter if configured
        local min_slider = item_enemy_min_counts[item_key]
        local min_required = (min_slider and min_slider:Get()) or 1
    ...
    if definition.type == "no_target" then
        if is_phoenix_supernova then
            -- Apenas simula apertar R - deixa o Umbrella fazer todo o resto
            if not cast_phoenix_supernova(hero, item) then
                return CAST_RESULT_NONE
            end
        else
    ...
    elseif definition.type == "position_enemy" then
        ...
        if definition.grenade_cast_offset and definition.grenade_cast_offset > 0 then
            local hero_pos = Entity.GetAbsOrigin(hero)
            if hero_pos then
                local direction = (target_pos - hero_pos):Normalized()
                local distance = hero_pos:Distance2D(target_pos)
                local offset = math.min(definition.grenade_cast_offset, distance * 0.5)
                cast_pos = hero_pos + direction * (distance - offset)
            end
        end

        Ability.CastPosition(item, cast_pos)
