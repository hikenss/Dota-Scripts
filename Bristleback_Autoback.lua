...
local ui = {}
ui.enabled = general_group:Switch("Ativar", true, HERO_ICON)
ui.radius = general_group:Slider("Raio de detecção de inimigos", 300, 2000, 900, "%d")
ui.min_enemies = general_group:Slider("Mínimo de inimigos próximos", 1, 5, 1, "%d")
ui.idle_delay = general_group:Slider("Atraso ocioso", 0, 2000, 350, function(value)
    return string.format("%d ms", value)
end)
ui.angle_tolerance = general_group:Slider("Tolerância de ângulo", 5, 90, 25, function(value)
    return string.format("%d°", value)
end)
ui.turn_distance = general_group:Slider("Distância de offset de giro", 15, 180, 70, "%d")
ui.hold_delay = general_group:Slider("Atraso de espera", 0, 600, 150, function(value)
    return string.format("%d ms", value)
end)

ui.order_cooldown = behavior_group:Slider("Atraso mínimo entre ordens", 0, 1000, 200, function(value)
    return string.format("%d ms", value)
end)
ui.pause_running = behavior_group:Switch("Pausar ao mover", true, HERO_ICON)
ui.pause_attacking = behavior_group:Switch("Pausar ao atacar", true, HERO_ICON)
ui.pause_casting = behavior_group:Switch("Pausar ao usar skill", true, HERO_ICON)
ui.pause_turning = behavior_group:Switch("Pausar ao já estar girando", true, HERO_ICON)
ui.sticky_time = behavior_group:Slider("Duração de travamento no alvo", 0, 3000, 600, function(value)
    return string.format("%d ms", value)
end)

ui.ignore_invisible = awareness_group:Switch("Ignorar inimigos invisíveis", true, HERO_ICON)
ui.include_illusions = awareness_group:Switch("Incluir ilusões", false, HERO_ICON)
ui.max_priority = awareness_group:Slider("Valor máximo de prioridade", 1, 10, 10, function(value)
    return string.format("≤ %d", value)
end)
ui.render_status = awareness_group:Switch("Renderizar status", false, HERO_ICON)

ui.enabled:ToolTip("Manter automaticamente as costas do Bristleback voltadas para inimigos próximos quando ocioso.")
ui.radius:ToolTip("Distância máxima para escanear heróis inimigos.")
...
local function passes_awareness_filters(enemy)
    ...
    if ui.ignore_invisible:Get() and not NPC.IsVisible(enemy) then
        return false
    end
