...
-- Menu padronizado igual ao DisruptorPos

local main_tab = Menu.Create("Heroes", "Hero List", "Disruptor", "Main Settings", "Combo Automático")
main_tab:Icon("panorama/images/heroes/icons/npc_dota_hero_disruptor_png.vtex_c")
local group = main_tab:Create("Combo Principal")

DisruptorCombo.optionEnable = group:Switch("Ativar combo automático", true)
DisruptorCombo.tp_back = group:Switch("Glimpse para retorno após TP", true)
DisruptorCombo.Find_enemy_radius = group:Slider("Distância para buscar inimigo", 50, 1000, 500)
DisruptorCombo.auto_kinetic_field = group:Switch("Auto Kinetic Field", true)
DisruptorCombo.Key2 = group:Bind("Tecla do Combo Completo", Enum.ButtonCode.BUTTON_CODE_NONE)

DisruptorCombo.Discord_Enable = group:Switch("Veil of Discord", true)
DisruptorCombo.Urn_Enable = group:Switch("Urna das Sombras", true)
DisruptorCombo.Spirit_Vessel_Enable = group:Switch("Spirit Vessel", true)
DisruptorCombo.Meteor_Enable = group:Switch("Meteor Hammer", true)

DisruptorCombo.Font = Render.LoadFont("Tahoma", 15, Enum.FontWeight.EXTRABOLD)
