...
-- Menu Setup (matches working pattern)
local hero_tab = Menu.Create("Heroes", "Hero List", "Lion")
local main_settings = hero_tab:Create("Main Settings")
local root = main_settings:Create("Combo")

local Toggle = root:Switch("Enable Auto Finger", false, "\u{f205}")
...
function LionAutoUlt.OnUpdate()
	...
	if finger ~= nil and finger ~= 0 and Ability.IsReady(finger) and Ability.GetManaCost(finger) <= NPC.GetMana(myHero) then
		...
		for _,hero in pairs(Heroes.GetAll()) do
			if hero ~= nil and hero ~= 0 and NPC.IsEntityInRange(myHero,hero,Ability.GetCastRange(finger)) and not Entity.IsSameTeam(hero,myHero) then
				...
				if Entity.IsAlive(hero) and not Entity.IsDormant(hero) and not NPC.IsIllusion(hero) and LionAutoUlt.IsHasGuard(hero) == "nil" then
					local totaldomage = LionAutoUlt.GetDamageDagon(myHero,hero,fingerDamage)
...
-- Game Events
function LionAutoUlt.OnGameStart()
	-- Reset on game start if needed
end

function LionAutoUlt.OnGameEnd()
	-- Cleanup on game end if needed
end
