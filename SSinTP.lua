...
do
  ...
  ui.enable_fallback = g:Switch("Enable", true, "\u{f0e7}")
  ui.only_enemy_tp = g:Switch("Only Enemy TPs", true)
  ui.auto_invoke = g:Switch("Auto Invoke SS", false)
  ui.min_mana = g:Slider("Minimum Mana %", 0, 100, 10, "%d")
  ui.cast_safety_ms = g:Slider("Safety Margin (ms)", 0, 300, 50, "%d")
  ui.net_ping = g:Slider("Ping (ms)", 0, 250, 40, "%d")
  ui.danger_range = g:Slider("Cancel if Enemies <", 0, 1200, 700, "%d")
  ui.draw_preview = g:Switch("Draw Preview", true, "\u{f5b0}")
  ui.debug = g:Switch("Debug", false)

  -- Fountain/TP Options (merging Sunstrike_On_Fountain)
  local gF = tab:Create("Fountain/TP")
  ui.fountain_strike = gF:Switch("Auto SS at Fountain (start)", true, "\u{f015}")
  ui.tp_fountain_strike = gF:Switch("SS on TP at Fountain (KILL)", true, "\u{f0e7}")
  ui.tp_anywhere_strike = gF:Switch("SS on TP anywhere", true, "\u{26a1}")
  ui.tp_damage_threshold = gF:Slider("Min. TP Damage %", 10, 80, 30, "%d")
  ui.min_visible_enemies = gF:Slider("Min. Visible Enemies (start)", 0, 5, 2, "%d")

  local g2 = tab:Create("Auto Sun Strike AI")
  ui.auto_ss_enabled = g2:Switch("Enable Auto SS AI", false, "\u{2600}")
  ui.ai_prediction = g2:Switch("Advanced Prediction", true)
  ui.ai_pattern_learning = g2:Switch("Pattern Learning", true)
  ui.ai_dodge_detection = g2:Switch("Dodge Attempt Detection", true)
  ui.auto_ss_channeling = g2:Switch("Target Channeling", true)
  ui.auto_ss_stun = g2:Switch("Target Stunned/Rooted", true)
  ui.auto_ss_stationary = g2:Switch("Target Stationary", true)
  ui.auto_ss_min_hp = g2:Slider("Min HP %", 0, 100, 15, "%d")
  ui.auto_ss_max_hp = g2:Slider("Max HP %", 0, 100, 85, "%d")
  ui.auto_ss_cooldown = g2:Slider("Interval (s)", 0, 10, 2, "%d")
  ui.auto_ss_range = g2:Slider("Max Range", 500, 5000, 2500, "%d")
  ui.auto_ss_draw = g2:Switch("Draw Indicator", true, "\u{f192}")
  ui.min_hit_chance = g2:Slider("Min Hit Chance %", 20, 95, 35, "%d")
  ui.aggressive_mode = g2:Switch("Aggressive Mode (risk more)", true)
  ui.ignore_danger = g2:Switch("Ignore Nearby Danger", false)
end
