local script = {}
local menu = Menu.Create("Heroes", "Hero List", "Shadow Shaman", "Main Settings", "Ward Settings")
local switch = menu:Switch("Control Serpent Wards", true, "\u{e1ec}")

local hero, player = nil, nil
local ward_targets = {}
local ward_last_order = {}
local bigWard = "npc_dota_shadowshaman_serpentward"

local function GetSerpentWards()
  local wards = {}
  for _, entity in pairs(Entities.GetAll()) do
    if Entity.IsAlive(entity)
        and Entity.GetUnitName(entity) == bigWard
...
local function FindBestTarget(ward)
  ...
  for _, enemy in ipairs(enemies) do
    ...
    elseif name == "npc_dota_courier" then
      return enemy -- try to kill enemy couriers
