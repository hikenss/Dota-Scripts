...
-- Funções auxiliares de timer
local function check_timer(name)
	return timers[name] and GameRules.GetGameTime() < timers[name]
end

local function set_timer(name, duration)
	timers[name] = GameRules.GetGameTime() + duration
end

local function is_enabled()
	return ui.enable:Get() and enabled_state
end

local function handle_toggle()
	if ui.enable_bind and ui.enable_bind:IsPressed() then
		if not check_timer("cs_toggle_cd") then
			enabled_state = not enabled_state
			set_timer("cs_toggle_cd", 0.25)
		end
	end
end
...
local function get_counterspell()
	...
	local cs = NPC.GetAbility(hero, "antimage_counterspell")
	if not cs then
		cs = NPC.GetAbility(hero, "antimage_counterspell_ally")
	end
	return cs
end
...
local function is_counterspell_ready()
	...
	if not Ability.IsCastable(cs, NPC.GetMana(hero)) then return false end
...
local function cast_counterspell()
	if check_timer("counterspell_cast_cd") then return false end
	
	local cs = get_counterspell()
	if not cs then return false end
	
	Ability.CastNoTarget(cs)
	last_cast_time = GameRules.GetGameTime()
	set_timer("counterspell_cast_cd", 0.1)
	return true
end
...
local function should_counterspell_for(proj)
	...
	if source and Entity.IsSameTeam(source, hero) then return false end

	if proj.target and proj.target ~= hero then return false end
...
return {
	...
	OnDraw = function()
		...
		local z = NPC.GetHealthBarOffset(hero, true) or 0
		local hb_world = hero_pos + Vector(0, 0, z + 135)
		...
		Render.Image(counterspell_img, pos, Vec2(size + 3, size), img_color, rounding)
	...
	OnUpdate = function()
		...
		-- Verifica timer agendado
		if check_timer("counterspell_scheduled") then
	...
	OnPrepareUnitOrders = function(data)
		...
		if not data.target or data.target ~= hero then return true end
		local caster = data.npc
		if caster and Entity.IsSameTeam(caster, hero) then return true end
		...
		if caster then
			...
			if not NPC.IsEntityInRange(caster, hero, eff_range + 50) then
				return true
			end
		end
		...
		if delay <= 0 then
			...
			set_timer("counterspell_scheduled", delay)
		end
	...
	OnUnitAnimation = function(data)
		...
		if not data or not data.unit or Entity.IsSameTeam(data.unit, hero) then return end
