        ...
        for team, data in pairs(State.deadCouriers) do
            ...
            if timeLeft > 0 then
                ...
                if data.ownerHero then
                    ...
                    if heroName then
                        if not State.heroIcons[heroName] then
                            State.heroIcons[heroName] = Render.LoadImage("panorama/images/heroes/icons/" .. heroName .. "_png.vtex_c")
                        end
                        if State.heroIcons[heroName] then
                            Render.Image(State.heroIcons[heroName], Vec2(x + 6, y + yOffset - 2), Vec2(16, 16), Color(255, 255, 255, 255))
                        end
                    end
                end
    ...
    for id, data in pairs(trackedCouriers) do
        if not currentCouriers[id] and (currentTime - data.lastSeen) > 1 then
            if data.team ~= myTeam then
                -- Encontra o dono do courier
                local ownerHero = nil
                local allHeroes = Heroes.GetAll()
                for _, hero in pairs(allHeroes) do
                    if hero and Entity.GetTeamNum(hero) == data.team then
                        ownerHero = hero
                        break
                    end
                end
