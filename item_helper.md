...
local ITEM_DB = {
    ...
    -- Modern items
    {name="item_echo_sabre",     display="Echo Sabre",       cost=2500,   phase={1,2},     tags={"mana","phys_dps","slow","str"},
     triggers={"squishy"}},
    {name="item_falcon_blade",   display="Falcon Blade",     cost=1125,   phase={1},       tags={"hp","mana","phys_dps"}},
    {name="item_khanda",         display="Khanda",           cost=5000,   phase={2,3},     tags={"phys_burst","magic_burst","crit"},
     triggers={"squishy","carry"}},
    {name="item_revenants_brooch",display="Revenant's Brooch",cost=5800,  phase={3},       tags={"magic_burst","int","vs_phys"},
     triggers={"armor_heavy","phys_dps"}},
    {name="item_meteor_hammer",  display="Meteor Hammer",    cost=2350,   phase={1,2},     tags={"stun","push","hp","mana","str","int"}},
    {name="item_ancient_janggo", display="Drum of Endurance",cost=1650,   phase={1,2},     tags={"ms","attack_speed","stats","aura"}},
    {name="item_parasma",        display="Parasma",          cost=5250,   phase={2,3},     tags={"magic_burst","int","slow"},
     triggers={"magic_resist"}},
}
...
local function computeItemScore(itemDef, ctx, withBreakdown)
    ...
    -- Soft phase gating: fora da fase = multiplicador 0.3 em vez de excluir totalmente
    local inPhase = tableContains(itemDef.phase, ctx.phase)
    if ctx.ownedItems and ctx.ownedItems[itemDef.name] then return -1, nil end
    -- Deduplicacao de itens aliados — se um aliado ja tem o item, grande penalidade
    if ctx.allyItemCounts and ctx.allyItemCounts[itemDef.name] and ctx.allyItemCounts[itemDef.name] > 0 then
        -- Itens de aura/team nao devem ser duplicados (Pipe, Crimson, Mek, Vlads, etc.)
        local isTeamItem = itemHasTag(itemDef, "team") or itemHasTag(itemDef, "save")
            or itemDef.name == "item_pipe" or itemDef.name == "item_crimson_guard"
            or itemDef.name == "item_mekansm" or itemDef.name == "item_guardian_greaves"
            or itemDef.name == "item_vladmir" or itemDef.name == "item_spirit_vessel"
            or itemDef.name == "item_solar_crest" or itemDef.name == "item_ancient_janggo"
        if isTeamItem then return -1, nil end
    end
    ...
    if itemDef.triggers then
        for _, tr in ipairs(itemDef.triggers) do
            ...
            if cnt > 0 then
                -- Diminishing returns: 1 hero=4, 2=7, 3=9, 4=10, 5=11
                local d = math.floor(4 * (1 + math.log(cnt) / math.log(2) * 0.4))
                score = score + d
    ...
    for _, rule in ipairs(COUNTER_RULES) do
        ...
        if matched then
            ...
            if itemMatch > 0 then
                -- Diminishing returns no totalEnemyTags
                local effectiveTags = math.max(1, math.floor(1 + math.sqrt(totalEnemyTags - 1) * 1.5))
                local d = itemMatch * rule.weight * effectiveTags
                score = score + d
    ...
    -- Soft phase gating: items fora da fase atual recebem penalidade em vez de serem excluidos
    if not inPhase and score > 0 then
        local before = score
        score = math.max(1, math.floor(score * 0.3))
        addBreak("out_of_phase", score - before)
    end
...
local function classifySuggestionCategory(itemDef, score, breakdown)
    ...
    if defensive and (score >= 15 or triggerPower >= 12 or enemyItemPower >= 8) then
        return "must_have"
    end
    if (counterPower + enemyItemPower) >= 14 and score >= 13 then
        return "must_have"
    end
...
local function analyzeEnemyTeam()
    ...
    -- Ally items (for deduplication of team/aura items)
    S.allyItemCounts = {}
    local allHeroes = Heroes.GetAll()
    if allHeroes then
        for _, allyHero in ipairs(allHeroes) do
            if allyHero ~= me then
                local aTeam = safeStatic(Entity, "GetTeamNum", allyHero)
                if aTeam and aTeam == myTeam then
                    local aAlive = safeStatic(Entity, "IsAlive", allyHero)
                    if aAlive ~= false then
                        local isIll = safeStatic(NPC, "IsIllusion", allyHero)
                        if not isIll then
                            for i = 0, 8 do
                                local aItem = safeStatic(NPC, "GetItemByIndex", allyHero, i)
                                if aItem then
                                    local aName = safeStatic(Ability, "GetName", aItem)
                                    if aName then
                                        S.allyItemCounts[aName] = (S.allyItemCounts[aName] or 0) + 1
                                    end
                                end
                            end
                            local aNeutral = safeStatic(NPC, "GetItemByIndex", allyHero, 16)
                            if aNeutral then
                                local anName = safeStatic(Ability, "GetName", aNeutral)
                                if anName then
                                    S.allyItemCounts[anName] = (S.allyItemCounts[anName] or 0) + 1
                                end
                            end
                        end
                    end
                ...
                if sg(UI.trackEnemyItems, true) then
                    ...
                    if neutralItem then
                        ...
                        if nName then
                            ...
                            S.enemyItemCounts[nName] = (S.enemyItemCounts[nName] or 0) + 1
                        end
                    end
                end
    ...
    local ctx = {
        ...
        allyItemCounts = S.allyItemCounts,
