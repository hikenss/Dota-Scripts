--[[
    ...
    ~~~ special thanks to: qww, shz and other [vetra] members d_b _codex_7
...
local function CreateUI(tab)
    ...
    return {
        ...
        PhantomRushHelper = {
            Enabled = mainGroup:Switch("Disable input during Phantom Rush", true, Config.UI.Icons.Rush),
            BlockDuration = mainGroup:Slider("Block duration (ms)", 0, 500, 250, "%d ms"),
            ToggleKey = mainGroup:Bind("Toggle Rush block", Enum.ButtonCode.KEY_NONE, Config.UI.Icons.Key)
        },
        Visuals = {
...
local function InitializeUI()
    -- Menu.Find("Heroes", "Hero List", "Phantom Lancer") + Helper tab with groups (API: Menu.Find -> :Create)
    ...
    -- Fallback: if Phantom Lancer is not found in Heroes, create own tree (General > PL Illusion Controller)
...
phantomRushBlockUntil = phantomRushBlockUntil or 0

local function GetPhantomRushStatusConfigPath()
    return "pl_rush_status.ini"
end

local function LoadPhantomRushStatusPosition()
    local configPath = GetPhantomRushStatusConfigPath()
    local file = io.open(configPath, "r")
    if file then
        for line in file:lines() do
            local x = line:match("pos_x=(%d+)")
            local y = line:match("pos_y=(%d+)")
            if x then PhantomRushStatusConfig.X = tonumber(x) end
            if y then PhantomRushStatusConfig.Y = tonumber(y) end
        end
        file:close()
    end
end

local function SavePhantomRushStatusPosition()
    local configPath = GetPhantomRushStatusConfigPath()
    local file = io.open(configPath, "w")
    if file then
        file:write(string.format("pos_x=%d\n", PhantomRushStatusConfig.X))
        file:write(string.format("pos_y=%d\n", PhantomRushStatusConfig.Y))
        file:close()
    end
end

local function HandlePhantomRushStatusInput()
    local cursorX, cursorY = Input.GetCursorPos()
    local isInStatus = cursorX >= PhantomRushStatusConfig.X and 
                      cursorX <= PhantomRushStatusConfig.X + PhantomRushStatusConfig.Width and
                      cursorY >= PhantomRushStatusConfig.Y and 
                      cursorY <= PhantomRushStatusConfig.Y + PhantomRushStatusConfig.Height

    if isInStatus and Input.IsKeyDown(Enum.ButtonCode.KEY_MOUSE1) and not PhantomRushStatusDrag.IsDragging then
        PhantomRushStatusDrag.IsDragging = true
        PhantomRushStatusDrag.StartX = cursorX
        PhantomRushStatusDrag.StartY = cursorY
        PhantomRushStatusDrag.OffsetX = cursorX - PhantomRushStatusConfig.X
        PhantomRushStatusDrag.OffsetY = cursorY - PhantomRushStatusConfig.Y
    end

    if PhantomRushStatusDrag.IsDragging then
        if Input.IsKeyDown(Enum.ButtonCode.KEY_MOUSE1) then
            PhantomRushStatusConfig.X = cursorX - PhantomRushStatusDrag.OffsetX
            PhantomRushStatusConfig.Y = cursorY - PhantomRushStatusDrag.OffsetY

            local screenSize = Render.ScreenSize()
            PhantomRushStatusConfig.X = math.max(0, math.min(PhantomRushStatusConfig.X, screenSize.x - PhantomRushStatusConfig.Width))
            PhantomRushStatusConfig.Y = math.max(0, math.min(PhantomRushStatusConfig.Y, screenSize.y - PhantomRushStatusConfig.Height))
        else
            SavePhantomRushStatusPosition()
            PhantomRushStatusDrag.IsDragging = false
        end
    end
...
-- Now DrawPhantomRushStatus can use it
local function DrawPhantomRushStatus()
    local myHero = Heroes.GetLocal()
    if not myHero then return end
    
    local isHovered = HandlePhantomRushStatusInput()
    
    local rushBlockEnabled = UI.PhantomRushHelper.Enabled:Get()
    local statusText = rushBlockEnabled and "Rush Block: ON" or "Rush Block: OFF"
    local statusColor = rushBlockEnabled and Color(120, 255, 120, 255) or Color(255, 120, 120, 255)
    local bgColor = rushBlockEnabled and Color(20, 40, 20, 220) or Color(40, 20, 20, 220)
    local bgColorHover = rushBlockEnabled and Color(25, 50, 25, 230) or Color(50, 25, 25, 230)
    
    local finalBgColor = isHovered and bgColorHover or bgColor
    
    -- Draw blur background
    DrawBlurredBackground(
        PhantomRushStatusConfig.X, 
        PhantomRushStatusConfig.Y, 
        PhantomRushStatusConfig.Width, 
        PhantomRushStatusConfig.Height, 
        PhantomRushStatusConfig.BorderRadius, 
        10, 
        0.91
    )
    ...
    -- Draw shadow
    Render.Shadow(
        Vec2(PhantomRushStatusConfig.X, PhantomRushStatusConfig.Y),
        Vec2(PhantomRushStatusConfig.X + PhantomRushStatusConfig.Width, PhantomRushStatusConfig.Y + PhantomRushStatusConfig.Height),
        Color(0, 0, 0, 255),
        24,
        PhantomRushStatusConfig.BorderRadius,
    ...
    -- Draw icon
    local icon = rushBlockEnabled and "\u{f05e}" or "\u{f204}"
    local iconSize = Render.TextSize(Config.Fonts.Main, 14, icon)
    local iconX = PhantomRushStatusConfig.X + 8
    local iconY = PhantomRushStatusConfig.Y + (PhantomRushStatusConfig.Height - iconSize.y) / 2
    
    Render.Text(Config.Fonts.Main, 14, icon, Vec2(iconX + 1, iconY + 1), Color(0, 0, 0, 80))
    Render.Text(Config.Fonts.Main, 14, icon, Vec2(iconX, iconY), statusColor)
    
    -- Draw separator
    local separatorX = iconX + iconSize.x + 8
    local separatorY = PhantomRushStatusConfig.Y + 4
    local separatorHeight = PhantomRushStatusConfig.Height - 8
    
    Render.FilledRect(
        Vec2(separatorX, separatorY),
        Vec2(separatorX + 2, separatorY + separatorHeight),
        Color(15, 15, 15, 70)
    )
    
    -- Draw status text
    local textSize = Render.TextSize(Config.Fonts.Main, 12, statusText)
    local textX = separatorX + 8
    local textY = PhantomRushStatusConfig.Y + (PhantomRushStatusConfig.Height - textSize.y) / 2
    
    Render.Text(Config.Fonts.Main, 12, statusText, Vec2(textX + 1, textY + 1), Color(0, 0, 0, 80))
    Render.Text(Config.Fonts.Main, 12, statusText, Vec2(textX, textY), statusColor)
    
    -- Show block duration when enabled and active
    if rushBlockEnabled and IsPhantomRushActive(myHero) then
        local now = GameRules.GetGameTime()
        local timeRemaining = math.max(0, phantomRushBlockUntil - now)
        if timeRemaining > 0 then
            local durationText = string.format("%.2fs", timeRemaining)
            local durationSize = Render.TextSize(Config.Fonts.Main, 10, durationText)
            local durationX = PhantomRushStatusConfig.X + PhantomRushStatusConfig.Width - durationSize.x - 8
            local durationY = PhantomRushStatusConfig.Y + (PhantomRushStatusConfig.Height - durationSize.y) / 2
            
            Render.Text(Config.Fonts.Main, 10, durationText, Vec2(durationX + 1, durationY + 1), Color(0, 0, 0, 80))
            Render.Text(Config.Fonts.Main, 10, durationText, Vec2(durationX, durationY), Color(255, 255, 120, 255))
        end
    end
end
...
local function IsPhantomRushActive(hero)
    if not hero or not Entity.IsNPC(hero) then return false end
    return NPC.HasModifier(hero, "modifier_phantom_lancer_phantom_edge_boost")
end

local function CheckAndBlockPhantomRush()
    -- Check if toggle key is pressed to enable/disable
    local toggleKey = UI.PhantomRushHelper.ToggleKey
    if toggleKey and toggleKey:IsPressed() then
        local currentState = UI.PhantomRushHelper.Enabled:Get()
        UI.PhantomRushHelper.Enabled:Set(not currentState)
    end
    
    if not UI.PhantomRushHelper.Enabled:Get() then
        return
    end
    ...
    if IsPhantomRushActive(myHero) then
        if now > phantomRushBlockUntil then
            local blockDuration = UI.PhantomRushHelper.BlockDuration:Get() / 1000
            phantomRushBlockUntil = now + blockDuration
        end
    end
end
...
function AutoBloodthorn()
    ...
    -- Don't interrupt Phantom Rush
    if IsPhantomRushActive(myHero) then return end
...
-- could be checked inline in the function; extracted for readability
...
function AutoFarmCreepsForIllusions()
    ...
    -- extra filtering for units still waiting to spawn
    local i = 1
    ...
    for _, illusion in ipairs(illusions) do
        ...
        if nearestCreep then
            ...
            if not farmLastOrder[id] or (now - farmLastOrder[id] > math.random(7,12)/10) then
                ...
                -- auto-lasthit logic may still need tuning.
                ...
                local armorReduction = 1 - (armorValue / (armorValue + 20)) -- armor reduction approximation
...
-- FIXME: this function is still buggy and not fully correct.
...
-- Wrote this function after a long break; implementation works but could be cleaner.
-- FIXME: it should split only the amount set in UI,
-- but currently if there are more illusions than the set max, it sends the full remainder.
        ...
        if enemy then
            ...
            for _, potentialTarget in ipairs(enemies) do
                ...
                if targetId ~= enemyId and Entity.IsAlive(potentialTarget) and not Entity.IsDormant(potentialTarget) then
                    ...
                    -- simple heuristic scoring for target redistribution
    ...
    for _, illusion in ipairs(illusions) do
        ...
        if isVisible then
            ...
            elseif bodyblockRole and bodyblockRole[id] then
                -- Check whether forced bodyblock is active
                local forceBodyblockKey = UI.AutoBodyblock.ForceBodyblockKey:Get()
                local isForceBodyblockPressed = forceBodyblockKey ~= Enum.ButtonCode.KEY_NONE and Input.IsKeyDown(forceBodyblockKey)
                if isForceBodyblockPressed then
            ...
            -- Using manual shadow draw here by preference.
...
local function GetIllusionStatus(id)
    ...
    -- optional; idle is not rendered anyway.
    return "I", "Idle"
end
...
-- Panel styling inspired by the Arc script, with a tweaked look.
...
PLTools.OnUpdate = function()
    ...
    if not isInitialized then
        LoadPanelPosition()
        LoadPhantomRushStatusPosition()  
        isInitialized = true
    end

    CleanupPlayerOverride()
    CheckAndBlockPhantomRush()
...
PLTools.OnPrepareUnitOrders = function(data)
    ...
    if UI.PhantomRushHelper.Enabled:Get() then
        local now = GameRules.GetGameTime()
        if now < phantomRushBlockUntil then
            return false
        end
