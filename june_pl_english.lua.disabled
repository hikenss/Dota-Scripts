--[[
      ...
      ~~~ special thanks to: qww, shz and other [ветра] members d_b
]]
...
local Config = {
    UI = {
        ...
        Groups = {
            ...
            AutoBait = "Isca Automática",
            AutoFollow = "Seguir Automático",
            AutoChase = "Perseguir Automático",
            AutoSplit = "Dividir Automático",
            RunAway = "Fugir"
        }
    },
    Colors = {
...
local function InitializeUI()
    ...
    return {
        AutoRush = {
            ...
            DisableRushOnFarm = mainGroup:Switch("Desabilitar Phantom Rush durante farm de creeps", false)
        },
        Visuals = {
            StatusText = illusionGroup:Switch("Mostrar status na tela", true)
        },
        Settings = {
            MinTriggerDist = settingsGroup:Slider("Distância para ativar o Rush", 300, 600, Config.Retreat.MinTriggerDist, "%d"),
            MinRetreat = settingsGroup:Slider("Recuo mínimo", 50, 300, Config.Retreat.MinRetreat, "%d"),
            MaxRetreat = settingsGroup:Slider("Recuo máximo", 300, 900, Config.Retreat.MaxRetreat, "%d"),
            RushSpeedThreshold = settingsGroup:Slider("Velocidade mínima para Rush", 250, 600, 300, "%d"),
            PlayerOverrideSec = settingsGroup:Slider("Tempo de controle manual (seg)", 0.0, 5.0, 1.5, "%.1f"),
        },
        RunAway = {
            HoldKey = runAwayGroup:Bind("Tecla de fuga/isca", Enum.ButtonCode.KEY_NONE),
            KeepCount = runAwayGroup:Slider("Ilusões próximas ao herói real", 0, 6, 2, "%d"),
            Duration = runAwayGroup:Slider("Duração da fuga (seg)", 0.5, 5.0, 2.0, "%.1f")
        },
        AutoBait = {
            Enabled = baitGroup:Switch("Ativar isca automática", true, Config.UI.Icons.Bait),
            HPThreshold = baitGroup:Slider("HP mínimo da ilusão (%)", 10, 90, math.floor(Config.Bait.HPThreshold * 100), "%d%%"),
            MaxBaiting = baitGroup:Slider("Máximo de ilusões iscando", 1, 5, Config.Bait.MaxBaiting, "%d"),
            Interval = baitGroup:Slider("Intervalo entre iscas (seg)", 1, 10, Config.Bait.Interval, "%d"),
            MinIllusions = baitGroup:Slider("Mínimo de ilusões para isca", 2, 6, Config.Bait.MinIllusions, "%d"),
            WRadius = baitGroup:Slider("Raio para isca com W", 400, 1200, Config.Bait.WRadius, "%d"),
            WBaitCount = baitGroup:Slider("Ilusões iscando após W", 1, 5, Config.Bait.WBaitCount, "%d"),
            WBaitInterval = baitGroup:Slider("Intervalo de isca com W (seg)", 0, 2, Config.Bait.WBaitInterval, "%.1f")
        },
        AutoFollow = {
            Enabled = followGroup:Switch("Ativar seguir automático", true),
            Count = followGroup:Slider("Ilusões que seguem o herói", 1, 3, Config.Follow.Count, "%d"),
            MinRadius = followGroup:Slider("Raio mínimo para seguir", 100, 350, Config.Follow.MinRadius, "%d"),
            MaxRadius = followGroup:Slider("Raio máximo para seguir", 200, 600, Config.Follow.MaxRadius, "%d")
        },
        AutoChase = {
            Enabled = chaseGroup:Switch("Ativar perseguição automática", true),
            Radius = chaseGroup:Slider("Raio de perseguição automática", 1500, 6000, 1600, "%d")
        },
        AutoBodyblock = {
            Enabled = illusionGroup:Switch("Ativar bodyblock automático", true),
            PingThreshold = illusionGroup:Slider("Limite de ping para bodyblock", 50, 300, 150, "%d"),
            ForceBodyblockKey = illusionGroup:Bind("Tecla de bodyblock", Enum.ButtonCode.KEY_NONE)
        },
        AutoSplit = {
            Enabled = splitGroup:Switch("Ativar ataque dividido automático", true),
            MaxIllusionsPerTarget = splitGroup:Slider("Máximo de ilusões por alvo", 3, 12, 8, "%d"),
            SplitCount = splitGroup:Slider("Ilusões para redirecionar", 1, 5, 2, "%d"),
            SearchRadius = splitGroup:Slider("Raio de busca de inimigos", 800, 2000, 1200, "%d"),
            MinEnemies = splitGroup:Slider("Mínimo de inimigos para ativação", 2, 5, 2, "%d")
        },
...
local function DrawIllusionStatus()
    ...
    for _, illusion in ipairs(illusions) do
        ...
        if isVisible then
            ...
            elseif state == "retreating" then
                text = string.format(
                    "Иллюзия: отходит (старт: %d, отход: %d, путь: %d, попыток: %d)",
            ...
            elseif state == "attacking" then
                ...
                text = string.format("Иллюзия: атакует врага (дист: %d)", math.floor(distToEnemy))
            elseif state == "done" then
                text = "Иллюзия: готово"
            else
                text = "Иллюзия: ожидает врага"
            end
...
local PanelColors = {
    ...
    StatusColors = {
        ["W-Байт"] = Color(255, 220, 0, 255),
        ["Байт"] = Color(255, 140, 0, 255),
