local tab = Menu.Create("Creeps", "Distributed");
local ui = tab:Create("Options");
tab:Icon("\u{f1bb}");
...
local autoMode = ui:Switch("Auto Mode (Select Units & Go)", false, "\u{f0c9}");
local debugDraw = ui:Switch("Debug Visualization", false, "");
...
local perStackShiftCenti = ui:Slider("Stack Time Shift (0.3-0.6 sec)", 30, 60, 40, function(v)
	return string.format("%.1f", v / 100);
end);
local fancyVisuals = ui:Switch("Visual Performance", true, "");
local minimapEnabled = ui:Switch("Show Minimap Overlay", true, "\u{f1bb}");
...
function OnUpdate()
	...
	if (#alive == 0) then
		print("[AutoStacker] All units dead, disabling bot.");
		isActive = false;
		return;
	end
	if (autoMode:Get() and #alive > 0) then
		for _, u in ipairs(alive) do
			if not unitToCampId[u] then
				local assignment = assignUnitsToNearestCamps({u});
				if assignment[u] then
					unitToCampId[u] = assignment[u];
				end
			end
		end
	end
	...
	for _, u in ipairs(alive) do
		...
		if not campId then
			print("[AutoStacker] Unit assignment not found.");
		else
			local points = getCampPoints(campId);
			...
			if (sec >= 50) then
				if ((campStacksMinute[campId] or -1) ~= minute) then
					...
					print(string.format("[AutoStacker] Camp #%d stacks detected: %d", campId, stacksNow));
				end
			end
			...
			if (sec < effFirstHit) then
				if not st.movedToWait then
					st.movedToWait = true;
					print(string.format("[AutoStacker] Unit %s at %d:%.2f -> WAIT (%.0f, %.0f, %.0f)", tostring(u), minute, sec, points.wait:GetX(), points.wait:GetY(), points.wait:GetZ()));
					moveUnitTo(player, u, points.wait);
				end
			else
				if ((sec >= effFirstHit) and (sec < (effFirstHit + 2)) and not st.attackIssued) then
					st.attackIssued = true;
				print(string.format("[AutoStacker] Unit %s at %d:%.2f -> ATTACK (%.0f, %.0f, %.0f) [first=%.1f, stacks=%d, eff=%.1f]", tostring(u), minute, sec, strike:GetX(), strike:GetY(), strike:GetZ(), baseHit, stacks, effFirstHit));
				attackMoveUnitTo(player, u, strike);
			end
			if (st.hitConfirmed and not st.pullIssued) then
				st.pullIssued = true;
				print(string.format("[AutoStacker] Unit %s at %d:%.2f -> PULL (%.0f, %.0f, %.0f) [on hit]", tostring(u), minute, sec, points.pull:GetX(), points.pull:GetY(), points.pull:GetZ()));
				moveUnitTo(player, u, points.pull);
			elseif ((sec >= 57) and (sec < 58) and not st.pullIssued) then
				st.pullIssued = true;
				print(string.format("[AutoStacker] Unit %s at %d:%.2f -> PULL (%.0f, %.0f, %.0f) [fallback]", tostring(u), minute, sec, points.pull:GetX(), points.pull:GetY(), points.pull:GetZ()));
					moveUnitTo(player, u, points.pull);
				end
			end
		end
...
function OnEntityHurt(data)
	...
	if (st.attackIssued and not st.hitConfirmed) then
		...
		if (points and not st.pullIssued) then
			st.pullIssued = true;
			print(string.format("[AutoStacker] Unit %s at %d:%02d -> PULL (%.0f, %.0f, %.0f) [OnEntityHurt]", tostring(src), minute, math.floor(t % 60), points.pull:GetX(), points.pull:GetY(), points.pull:GetZ()));
...
function OnDraw()
	...
	if (minimapEnabled:Get() and isActive and #selectedUnits > 0) then
		local minimapX, minimapY = 10, 100;
		local minimapW, minimapH = 240, 240;
		Render.FilledRect(Vec2(minimapX, minimapY), Vec2(minimapX + minimapW, minimapY + minimapH), Color(20, 20, 25, 200), 8);
		Render.Rect(Vec2(minimapX, minimapY), Vec2(minimapX + minimapW, minimapY + minimapH), Color(100, 100, 120, 220), 8, Enum.DrawFlags.RoundCornersAll, 1);
		local mapImg = Render.LoadImage("C:/Users/edcfa/Downloads/Umbrela/scripts/map.jpg");
		if mapImg then
			Render.Image(mapImg, Vec2(minimapX + 4, minimapY + 4), Vec2(minimapW - 8, minimapH - 8), Color(255, 255, 255, 180), 6, Enum.DrawFlags.RoundCornersAll);
		end
		local ay, az, aA, aB = 1e18, 1e18, -1e18, -1e18;
		for campId, _ in pairs(CAMP_POINTS) do
			local pts = getCampPoints(campId);
			if pts then
				local w = pts.wait;
				if w:GetX() < ay then ay = w:GetX() end;
				if w:GetY() < az then az = w:GetY() end;
				if w:GetX() > aA then aA = w:GetX() end;
				if w:GetY() > aB then aB = w:GetY() end;
			end
		end
		if ay < aA and az < aB then
			for campId, _ in pairs(CAMP_POINTS) do
				local pts = getCampPoints(campId);
				if pts then
					local w = pts.wait;
					local bv = (w:GetX() - ay) / (aA - ay);
					local x = (w:GetY() - az) / (aB - az);
					if bv < 0 then bv = 0 elseif bv > 1 then bv = 1 end;
					if x < 0 then x = 0 elseif x > 1 then x = 1 end;
					local px = minimapX + 4 + bv * (minimapW - 8);
					local py = minimapY + 4 + (1 - x) * (minimapH - 8);
					local col = (unitToCampId and next(unitToCampId) and Color(100, 200, 100, 255)) or Color(150, 150, 180, 200);
					Render.FilledCircle(Vec2(px, py), 4, col);
				end
			end
		end
	end
